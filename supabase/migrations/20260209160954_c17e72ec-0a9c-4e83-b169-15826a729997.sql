...
-- Update update_user_streak to apply 10% bonus for Pro users
...
BEGIN
  ...
  PERFORM award_points(_user_id, v_base_points, 'Pickup completion');

  -- Apply 10% Pro bonus (1 extra point per pickup)
  IF v_tier = 'pro' THEN
    v_pro_bonus := GREATEST(1, ROUND(v_base_points * 0.10));
    PERFORM award_points(_user_id, v_pro_bonus, 'Premium Pro 10% bonus');
  END IF;
