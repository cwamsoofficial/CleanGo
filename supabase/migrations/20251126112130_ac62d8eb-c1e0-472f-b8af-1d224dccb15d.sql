-- Fix the validate_admin_key function to properly handle digest
CREATE OR REPLACE FUNCTION public.validate_admin_key(input_key text, user_email text, user_ip text DEFAULT NULL::text)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_result JSON;
BEGIN
  -- Hash the provided key for comparison using proper casting
  v_key_hash := encode(digest(input_key::bytea, 'sha256'::text), 'hex');
